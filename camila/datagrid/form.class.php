...
/* This File is part of Camila PHP Framework
  ...
  require_once(CAMILA_LIB_DIR.'validator/Validator.php');
  ...
  class phpform
  {
      ...
      function add(&$field)
      {
          if (in_array($field->field, $this->keys) || !isset($_REQUEST['camila_inline_field']) || (isset($_REQUEST['camila_inline_field']) && ($_REQUEST['camila_inline_field'] == $field->field || $_REQUEST['camila_inline_field'] == $field->inline_field)))
      ...
      function draw_header()
      {
          ...
          if ($this->drawheadersubmitbutton) {
              ...
              if ($this->mode == 'insert') {
                  $js = new CHAW_js('<input type="submit" name="submitandnew_button_header" value="'.camila_get_translation('camila.insertandnewbutton').'" id="hawinputsubmitandnewheader" />');
                  $this->form->add_userdefined($js);
              }

              if (isset($_REQUEST['camila_delete']) && $this->mode != 'insert')
                  $js = new CHAW_js('<input type="submit" name="submit_button_header" value="'.camila_get_translation('camila.deletebutton').'" id="hawinputsubmitheader" /><br /><br />');
              else
                  $js = new CHAW_js('<input type="submit" name="submit_button_header" value="'.camila_get_translation('camila.' . $this->mode . 'button').'" id="hawinputsubmitheader" /><br /><br />');
      ...
      function draw_errors()
      {
          ...
          if (!is_array($value)) {
              ...
              $errtext .= isUTF8($this->fields[$key]->title) ? $this->fields[$key]->title : utf8_encode($this->fields[$key]->title) . camila_get_translation('camila.postformfielderror');
