...
if (camila_form_in_update_mode(worktable_worktable7)) {
    ...
    $form = new dbform('worktable_worktable7', 'id');
...
if (is_object($form->fields['codicebadge'])) $form->fields['codicebadge']->defaultvalue = worktable_parse_default_expression('${prefissocodiceabarre}${codice riga}', $form);
    ...
    $form->draw();
      ...
      $stmt = 'select ' . $report_fields . ' from worktable_worktable7';
      $report = new report($stmt.$filter, '', 'cognome', 'asc', $mapping, null, 'id', 'cf_bool_is_special,cf_bool_is_selected,tenda,cognome,nome,motivazione,accessodal,accessoal,codicebadge,note', '', (isset($_REQUEST['camila_rest'])) ? false : true, (isset($_REQUEST['camila_rest'])) ? false : true);
